...
subroutine get_los_coordinates()
  ...
  if (use_random_los) then 
    ...
    !
    number_of_LOS = nspec
    allocate(x_fraction_array(number_of_LOS),y_fraction_array(number_of_LOS),z_fraction_array(number_of_LOS),axis_array(number_of_LOS),&
    ...
    !
     ...
     IF (.NOT. use_random_los .AND. use_snapshot_file ) THEN
		...
		phi_array(number_of_LOS),theta_array(number_of_LOS),ncontribute(number_of_LOS),ncontribute_global(number_of_LOS))
     	...
     	DO i = 1, number_of_LOS
           READ(77,*) x_fraction_array(i), y_fraction_array(i), z_fraction_array(i), axis_array(i)
     	ENDDO
     	WRITE(*,*) '### NO LONGER enforcing the LOS to be along the z axis ###'	
!!!    	z_fraction_array = 0 ! BDO TOOK OUT
	...
	ENDIF
     ...
     call hdf5_read_data(file_handle, 'Projection/z_fraction_array', proj)
     z_fraction_array = proj(1:nspec)
     deallocate(proj)
     call hdf5_close_file(file_handle)
!!!     z_fraction_array = 0 ! BDO took out
     phi_array   = 0
...
subroutine projectdata(file_number)
  ...
  do ion=1, nion
    if(trim(ions(ion)) .eq. 'h1') h1_index = ion
    if(trim(ions(ion)) .eq. 'c2') c2_index = ion
    if(trim(ions(ion)) .eq. 'c3') c3_index = ion
    if(trim(ions(ion)) .eq. 'c4') c4_index = ion
    if(trim(ions(ion)) .eq. 'o6') o6_index = ion
    if(trim(ions(ion)) .eq. 'o7') o7_index = ion
    if(trim(ions(ion)) .eq. 'o8') o8_index = ion
    if(trim(ions(ion)) .eq. 'mg2') mg2_index = ion
    if(trim(ions(ion)) .eq. 'si3') si3_index = ion
    if(trim(ions(ion)) .eq. 'si4') si4_index = ion
    if(trim(ions(ion)) .eq. 'n5') n5_index = ion
  enddo
#endif
  !
  ! Initialize arrays
  ...
  met_ion   = 0.0  ! number dens weighted metallicity
  ...
  !
  ! Rotate and shift particles relative to sightline
  ...
  if (axis .eq. 2) then
    x_physical_hold = x_physical
    y_physical_hold = y_physical
    z_physical_hold = z_physical
  endif
  if (axis .eq. 1) then
    y_physical_hold = x_physical
    x_physical_hold = z_physical
    z_physical_hold = y_physical
  endif
  if (axis .eq. 0) then
    x_physical_hold = y_physical
    y_physical_hold = z_physical
  ...
  particle_loop: do i = 1, NGas
    ...
    zz = zz - z_physical_hold + box_2
    if(zz .gt. box_2) zz = zz - box
    if(zz .lt. -box_2) zz = zz + box
    ...
    hinv3 = hinv2 / hh
    !
    dx = abs(xx - x_physical_hold) 
    dy = abs(yy - y_physical_hold) 
    if(dx .gt. box_2) then 
      dx = box - dx 
    endif 
    if(dy .gt. box_2) then 
...
! 100 format(I16,1x,4(e12.4,1x))
...
#ifdef CHEMARRAY	
       if(h1_index .gt. 0) ionfrac(h1_index)  = HI(i)
       if(c2_index .gt. 0) ionfrac(c2_index)  = CII(i)
       if(c3_index .gt. 0) ionfrac(c3_index)  = CIII(i)
       if(c4_index .gt. 0) ionfrac(c4_index)  = CIV(i)
       if(o6_index .gt. 0) ionfrac(o6_index)  = OVI(i)
       if(o7_index .gt. 0) ionfrac(o7_index)  = OVII(i)
       if(o8_index .gt. 0) ionfrac(o8_index)  = OVIII(i)
       if(si2_index .gt. 0) ionfrac(si2_index)  = SiII(i)
       if(si3_index .gt. 0) ionfrac(si3_index)  = SiIII(i)
       if(si4_index .gt. 0) ionfrac(si4_index)  = SiIV(i)
       if(n5_index .gt. 0) ionfrac(n5_index)  = NV(i)
       if(mg2_index .gt. 0) ionfrac(mg2_index)  = MgII(i)
#endif
        !
	  ...
	  if (axis .eq. 2) vr = Velocity(3,i) ! peculiar velocity in km/s !!!
	  if (axis .eq. 1) vr = Velocity(2,i) ! peculiar velocity in km/s !!!
	  if (axis .eq. 0) vr = Velocity(1,i) ! peculiar velocity in km/s !!!
        endif
        !
  ...
  do ii = 1, nion
    do i = 1, nveloc
      if (n_ion(ii,i) .gt. 0.) then 
        ...
        met_ion(ii,i) = met_ion(ii,i) / n_ion(ii,i)
      endif
...
subroutine makespectra()
  ...
  do ion = 1, nion
    ...
    call computespectrum(nveloc,ion,vhubble,rho_tot,cdens,veloc_ion(ion,:),n_ion(ion,:), &
        temp_ion(ion,:),met_ion(ion,:),turbulence,tau,velocw,nionw,rhow,tempw,metw)
    !
    tau_ion(ion,:)     = tau(:)
...
subroutine computespectrum(nveloc,ion,vhubble,rho_tot,cdens,vpecul,nion,temperature,metallicity,turbulence,&
     tau,velocw,nionw,rhow,tempw,metw)
  use numbers
  use atomic_data
  ...
  real(kind=doubleR), intent(in)    :: vhubble(nveloc),rho_tot(nveloc),cdens(nveloc), &
       vpecul(nveloc),nion(nveloc),temperature(nveloc),metallicity(nveloc),turbulence(nveloc) 
  real(kind=doubleR), intent(out)   :: tau(nveloc), rhow(nveloc), tempw(nveloc), &
       metw(nveloc), velocw(nveloc), nionw(nveloc)
  !
  ! local variabales  
  ...
  ! metw         ! Optical depth weighted metallicity
      ...
      if (tauc .ge. taumin) then 
        ...
        if (.not. limsigma) then
          ...
          do j = 1,nveloc
            ...
            metw(j) = metw(j) + dtau * metallicity(i) 
            !
          enddo
        else
          ...
          do j = iveloc-noff,iveloc+noff
            ...
            metw(ispec) = metw(ispec) + dtau * metallicity(i) 
...
subroutine allocate_spectra_short()
  ...
  if(allocated(met_ion)) deallocate(met_ion)
  ...
  if(allocated(metw)) deallocate(metw)
  ...
  if(allocated(Met_Z_ion)) deallocate(Met_Z_ion)
  ...
  allocate(n_ion(nion,nveloc),temp_ion(nion,nveloc),met_ion(nion,nveloc),veloc_ion(nion,nveloc))
  allocate(tau(nveloc),normw(nveloc),work(nveloc),work2(nveloc),vhubble(nveloc),cdens(nveloc),&
    rhow(nveloc),tempw(nveloc),metw(nveloc),voc(nveloc),vocsim(nveloc),turbulence(nveloc),velocw(nveloc),nionw(nveloc))
  allocate(tau_ion(nion,nveloc),flux_ion(nion,nveloc))
  allocate(cdens_ion_integrated(nion))
  allocate(Rho_Z_ion(nion,nveloc),Temp_Z_Ion(nion,nveloc),Met_Z_ion(nion,nveloc),veloc_Z_ion(nion,nveloc),nion_Z_ion(nion,nveloc))
...
subroutine update_short_spectrum(particlefile, nlos)
  ...
  character(len=300)     :: GroupName, VarName, MassWeightedGroup, ElementGroup
...
subroutine write_short_spectrum(particlefile, los_number, nlos)
  ...
  if(ispec == 0) then
    ...
    call hdf5_write_data(file_handle,'Projection/z_fraction_array',z_fraction_array)
    call hdf5_write_data(file_handle,'Projection/axis_array',axis_array)
...
!!$  write (*,*) ' MyPE= ',MyPE,' rhocb= ',rhocb
  ...
  if (output_zspaceopticaldepthweighted_values) then
    do ion=1,nion
      ...
      VarName = trim(ElementGroup)//'/'//'Metallicity'
      call hdf5_write_data(file_handle, trim(VarName),met_z_ion(ion,:))
...
subroutine write_long_spectrum()
  ...
  character(len=300)    :: outfile, GroupName, ThisSpectrum, VarName, ElementGroup,inputfile
  !
  call hdf5_open_file(file_handle,trim(SpectrumFile))
  !
  if(ispec == 1) then
...
subroutine readdata_owls(filename,los_number)
  ...
  character(len=300)       :: basefile, longfile
  integer                  :: file_handle
  character(len=300)       :: LosNumber, LosName, VarName
  character(len=3)         :: FileNumber
  integer                  :: nfiles
...
subroutine read_full_snapshot()
  ...
  !
  ! local variables
  character(len=300)       :: basefile, urchin_basefile, longfile, urchin_longfile
...
#else
     !HACK for snipshots, have to assume SFR = 0.
     StarFormationRate(:) = 0.0*ParticleDensity(:)
#endif
     !
#ifndef SNIPSHOT
     !HACK for snipshots, no smoothing length so calculate from density and mass further down.
...
#ifdef CHEMARRAY
     ! Chemistry Array
     VarName = trim('PartType0')//'/ChemicalAbundances/'//'HydrogenI'
     call hdf5_read_data(file_handle,trim(VarName),HI(NTotal+1:NTotal+Npart_this_file))       	         

     VarName = trim('PartType0')//'/ChemicalAbundances/'//'CarbonII'
     call hdf5_read_data(file_handle,trim(VarName),CII(NTotal+1:NTotal+Npart_this_file))

     VarName = trim('PartType0')//'/ChemicalAbundances/'//'CarbonIII'
     call hdf5_read_data(file_handle,trim(VarName),CIII(NTotal+1:NTotal+Npart_this_file))

     VarName = trim('PartType0')//'/ChemicalAbundances/'//'CarbonIV'
     call hdf5_read_data(file_handle,trim(VarName),CIV(NTotal+1:NTotal+Npart_this_file))

     VarName = trim('PartType0')//'/ChemicalAbundances/'//'OxygenVI'
     call hdf5_read_data(file_handle,trim(VarName),OVI(NTotal+1:NTotal+Npart_this_file))

     VarName = trim('PartType0')//'/ChemicalAbundances/'//'OxygenVII'
     call hdf5_read_data(file_handle,trim(VarName),OVII(NTotal+1:NTotal+Npart_this_file))

     VarName = trim('PartType0')//'/ChemicalAbundances/'//'OxygenVIII'
     call hdf5_read_data(file_handle,trim(VarName),OVIII(NTotal+1:NTotal+Npart_this_file))

     VarName = trim('PartType0')//'/ChemicalAbundances/'//'SiliconII'
     call hdf5_read_data(file_handle,trim(VarName),SiII(NTotal+1:NTotal+Npart_this_file))

     VarName = trim('PartType0')//'/ChemicalAbundances/'//'SiliconIII'
     call hdf5_read_data(file_handle,trim(VarName),SiIII(NTotal+1:NTotal+Npart_this_file))

     VarName = trim('PartType0')//'/ChemicalAbundances/'//'SiliconIV'
     call hdf5_read_data(file_handle,trim(VarName),SiIV(NTotal+1:NTotal+Npart_this_file))

     VarName = trim('PartType0')//'/ChemicalAbundances/'//'NitrogenV'
     call hdf5_read_data(file_handle,trim(VarName),NV(NTotal+1:NTotal+Npart_this_file))

     VarName = trim('PartType0')//'/ChemicalAbundances/'//'MagnesiumII'
     call hdf5_read_data(file_handle,trim(VarName),MgII(NTotal+1:NTotal+Npart_this_file))


     write(*,*) 'HI(Ntotal+1) = ', HI(Ntotal+1), 'OVI(Ntotal+1) = ', OVI(Ntotal+1),&
     		'ParticleDensity(Ntotal+1) = ', ParticleDensity(Ntotal+1),&
     		'ParticleTemperature(Ntotal+1) = ', ParticleTemperature(Ntotal+1)
...
!!!  ParticleSmoothingLength  = ParticleSmoothingLength * Coordinates_conv ! MOVED TO LATER BECAUSE OF SNIPSHOT
  !
  Velocity_conv = Vel_aexp_exp * log10(ExpansionFactor) + vel_h_exp * log10(HubbleParam) + log10(vel_cgs_unit) & 
       - log10(1.d5) ! physical km/s
  Velocity_conv = 10.d0**Velocity_conv
  ...
  !
  if(HubbleParam .eq. 0) call abortrun('HubbleParam = 0!')
  Temp_conv          = 0.0 !!!Temp_aexp_exp * log10(ExpansionFactor) + Temp_h_exp * log10(HubbleParam) + log10(Temp_cgs_unit) ! cgs
  Temp_Conv          = 10.d0**Temp_Conv
  ParticleTemperature        = ParticleTemperature * Temp_Conv 
  !
  if(ignore_starforming) then
  ...
  !
#ifdef CHEMARRAY
     write(*,*) "Processing Chem Arrays"

     CII(:) = CII(:)/(MassFractions(C_index,:)/(12.0*MassFractions(H_index,:))+1e-20)
     write(*,*) "CII(1)= ", CII(1)
     CIII(:) = CIII(:)/(MassFractions(C_index,:)/(12.0*MassFractions(H_index,:))+1e-20)
     write(*,*) "CIII(1)= ", CIII(1)
     CIV(:) = CIV(:)/(MassFractions(C_index,:)/(12.0*MassFractions(H_index,:))+1e-20)
     write(*,*) "CIV(1)= ", CIV(1)
     OVI(:) = OVI(:)/(MassFractions(O_index,:)/(16.0*MassFractions(H_index,:))+1e-20)
     write(*,*) "OVI(1)= ", OVI(1)
     OVII(:) = OVII(:)/(MassFractions(O_index,:)/(16.0*MassFractions(H_index,:))+1e-20)
     OVIII(:) = OVIII(:)/(MassFractions(O_index,:)/(16.0*MassFractions(H_index,:))+1e-20)
     SiII(:) = SiII(:)/(MassFractions(Si_index,:)/(28.0*MassFractions(H_index,:))+1e-20)
     write(*,*) "SiII(1)= ", SiII(1)
     SiIII(:) = SiIII(:)/(MassFractions(Si_index,:)/(28.0*MassFractions(H_index,:))+1e-20)
     SiIV(:) = SiIV(:)/(MassFractions(Si_index,:)/(28.0*MassFractions(H_index,:))+1e-20)
     write(*,*) "SiIV(1)= ", SiIV(1)
     NV(:) = NV(:)/(MassFractions(N_index,:)/(14.0*MassFractions(H_index,:))+1e-20)
     write(*,*) "NV(1)= ", NV(1)
     MgII(:) = MgII(:)/(MassFractions(Mg_index,:)/(24.0*MassFractions(H_index,:))+1e-20)
     write(*,*) "MgII(1)= ", MgII(1)
#endif

#ifdef SNIPSHOT
     !!!write(*,*) "PartSmoothLength= ", ParticleSmoothingLength(1), ParticleSmoothingLength(2), ParticleSmoothingLength(3), ParticleSmoothingLength(4), ParticleSmoothingLength(5)
     ! Particle Smoothing HACK calculated from density and mass.  BDO
     ParticleSmoothingLength(:) = (ParticleDensity(:)*proton_mass/dens_cgs_unit/(Mass(:)*solar_mass/mass_cgs_unit))**(-0.3333)*1.78 !!*2.39
     write(*,*) "PartSmoothLengthHack= ", ParticleSmoothingLength(1), ParticleSmoothingLength(2), ParticleSmoothingLength(3), ParticleSmoothingLength(4), ParticleSmoothingLength(5)
     !!!write(*,*) "ParticleDensity= ", ParticleDensity(1), ParticleDensity(2), ParticleDensity(3), ParticleDensity(4), ParticleDensity(5)
     !!!write(*,*) "Mass= ", Mass(1), Mass(2), Mass(3), Mass(4), Mass(5)     
#endif
  ParticleSmoothingLength  = ParticleSmoothingLength * Coordinates_conv
  write(*,*) 'ParticleSmoothingLength(1) = ', ParticleSmoothingLength(1)
  write(*,*) 'ParticleSmoothingLength(2) = ', ParticleSmoothingLength(2)
  write(*,*) 'Dens(1) + Mass(1) = ', ParticleDensity(1)*proton_mass/dens_cgs_unit, Mass(1)*solar_mass/mass_cgs_unit;
  write(*,*) 'Dens(2) + Mass(2) = ', ParticleDensity(2)*proton_mass/dens_cgs_unit, Mass(2)*solar_mass/mass_cgs_unit;
...
subroutine allocate_particledata
  ...
  if(Ngas .ne. nGas_old)then
     ...
     if(allocated(CIV)) deallocate(CIV)
     if(allocated(OVI)) deallocate(OVI)
     if(allocated(OVII)) deallocate(OVII)
     if(allocated(OVIII)) deallocate(OVIII)
     if(allocated(SiII)) deallocate(SiII)
     if(allocated(SiIII)) deallocate(SiIII)
     if(allocated(SiIV)) deallocate(SiIV)
     if(allocated(NV)) deallocate(NV)
     if(allocated(MgII)) deallocate(MgII)
#endif
     !
     ...
     allocate(CIII(NGas),stat=AllocateStatus)
     allocate(CIV(NGas),stat=AllocateStatus)
     allocate(OVI(NGas),stat=AllocateStatus)
     allocate(OVII(NGas),stat=AllocateStatus)
     allocate(OVIII(NGas),stat=AllocateStatus)
     allocate(SiII(NGas),stat=AllocateStatus)
     allocate(SiIII(NGas),stat=AllocateStatus)
     allocate(SiIV(NGas),stat=AllocateStatus)
     allocate(NV(NGas),stat=AllocateStatus)
     allocate(MgII(NGas),stat=AllocateStatus)
#endif 
     if(AllocateStatus /= 0) &
...
subroutine load_ionbal(ion)
  ...
  character(len=300) :: longfile, VarName, filename
