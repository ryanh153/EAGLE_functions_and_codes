...
def populate_hdf5_with_matching_gals(cos_comparisons_file, directory_with_gal_folders, starting_gal_id, real_data_to_match, real_survey_radii, tols, hi_lo_tols, names_in_gal_outputs, which_survey, combined_plots_folder, spec_output_directory, cos_halos_bool, cos_dwarfs_bool, cos_gass_bool, cos_AGN_bool, cos_gto_bool):
	...
	plt.tight_layout()
	plt.hold(False)
	ax.set_xticklabels(ax.get_xticklabels(),fontsize=15) # these labelsizes don't appear to work. Why not? 
...
def get_EAGLE_data_for_plots(ion, rest_wavelength, cos_id_arr, lambda_line, spec_output_directory, lookup_file, max_smass, min_smass, max_ssfr, min_ssfr, tols, hi_lo_tols, ordered_cos_radii, covering_frac_bool, covering_frac_val, max_abs_vel, mass_estimates_bool, kinematics_bool, make_realistic_bool, offset = 0):
	...
	single_line_temp_bins = np.arange(4.0,8.0,0.25)
	single_line_max_plots = 3
	single_line_num_plotted = np.zeros(np.size(single_line_temp_bins))


	covered = 0
	total = 0
	directory_with_COS_LSF = '/projects/ryho3446/snapshots/COS_LSF_files/'
					...
					if gal_passes == True:
						...
						with h5py.File(spec_output_file,'r') as hf:
							...
							if kinematics_bool:
								...
								num_minima, centroid_vel, FWHM, depth, temp, line_ion_density, line_nH, single_line_num_plotted = get_line_kinematics(flux[indices], spec_hubble_velocity[indices], temperature[indices], density[indices], nH[indices], optical_depth[indices], i+offset, spec_num, radius, gal_mass, gal_ssfr, col_dense, single_line_temp_bins, single_line_max_plots, single_line_num_plotted, make_realistic_bool=make_realistic_bool, rest_wavelength=rest_wavelength, redshift=redshift, directory_with_COS_LSF=directory_with_COS_LSF)
...
def make_col_dense_plots(ion, covered, total, ssfr, masses, smasses, radii, virial_radii, cols, eagle_ids, cos_id_arr, plot_cols, plot_cols_err, plot_cols_flags, plot_cols_radii, covering_frac_val, colorbar,bins_for_median):
	...
	if np.size(plot_cols) > 0:
		...
		plt.hold(False)
		ax.set_title('COS-Halos Passive: N vs b for HI')

	else:
		if colorbar == 'smass':
			plt.scatter(radii, cols, s=20., c=np.log10(smasses), cmap='RdBu_r', edgecolor = 'black', linewidth = 0.5, label = 'EAGLE (black fit)')
			cb = plt.colorbar()
			cb.set_label(r'$log_{10}(M_{star})$')
		elif colorbar == 'hmass':
			plt.scatter(radii, cols, s=20., c=np.log10(masses), cmap='RdBu_r', edgecolor = 'black', linewidth = 0.5, label = 'EAGLE (black fit)')
			cb = plt.colorbar()
			cb.set_label(r'$log_{10}(M_{200})$')
		elif colorbar == 'ssfr':
			plt.scatter(radii, cols, c=ssfr, s=20., cmap='RdBu', edgecolor = 'black', linewidth = 0.5, label = 'EAGLE (black fit)')
			cb = plt.colorbar()
		...
		plt.title('COS-Halos Passive: N vs b for HI')
	...
	if colorbar == 'smass':
		fig.savefig(ion+'col_with_color_smass.pdf', bbox_inches='tight')
	elif colorbar == 'hmass':
		fig.savefig(ion+'col_with_color_hmass.pdf', bbox_inches='tight')
	elif colorbar == 'ssfr':
		fig.savefig(ion+'col_with_color_ssfr.pdf', bbox_inches='tight')
	plt.close()
...
def make_equ_width_plots(ion, ssfr, masses, smasses, radii, virial_radii, equ_widths, eagle_ids, cos_id_arr, plot_equ_widths, plot_W_errs, plot_W_flags, plot_equ_widths_radii, cos_smass_data, colorbar, bins_for_median, log_plots):
	...
	if np.size(plot_equ_widths) > 0:
		...
		ax.set_title('COS-Halos Passive: W vs b for %s' % (ion.upper()))
		# ax.add_artist(fit_legend)
		ax.add_artist(data_legend)
		...
		ax.set_title('COS-Halos Passive: W vs b')
	...
	if colorbar == 'smass':
		fig.savefig(ion+'_equ_width_with_color_smass.pdf', bbox_inches='tight')
	elif colorbar == 'hmass':
		fig.savefig(ion+'_equ_width_with_color_hmass.pdf', bbox_inches='tight')
	elif colorbar == 'ssfr':
		fig.savefig(ion+'_equ_width_with_color_ssfr.pdf', bbox_inches='tight')
	plt.close(fig)
	...
	try:
		plt.title('COS-Halos Passive: W vs b for %s' % (ion.upper()))
	except:
		plt.title('COS-Halos Passive: W vs b for %s' % (ion.upper()))
	plt.xlabel('Impact Parameter (in virial radii)')
	plt.ylabel(r'Equivalent Width ($\AA{}$)')
	if colorbar == 'smass':
		plt.savefig(ion+'equ_width_virial_with_color_smass.pdf', bbox_inches='tight')
	elif colorbar == 'hmass':
		plt.savefig(ion+'equ_width_virial_with_color_hmass.pdf', bbox_inches='tight')
	elif colorbar == 'ssfr':
...
def make_contour_col_dense_plots(ion, radii, virial_radii, cols, plot_cols, plot_cols_err, plot_cols_flags, plot_cols_radii, smasses, ssfr, virial_radii_bool):
	...
	plt.title('COS-Halos Passive: N vs b for %s' % (ion.upper()), fontsize=16.)
...
def make_equ_width_contour_plots(ion, radii, virial_radii, equ_widths, smasses, ssfr, plot_equ_widths, plot_W_errs, plot_W_flags, plot_equ_widths_radii, curr_cos_smass, curr_cos_ssfr, virial_radii_bool, log_plots):
	...
	plt.rcParams['axes.labelsize'], plt.rcParams['axes.titlesize'], plt.rcParams['legend.fontsize'], plt.rcParams['xtick.labelsize'], plt.rcParams['ytick.labelsize'] = 18, 20., 15, 14, 14
	...
	ax.set_title(r'COS-Halos Passive: $W_{HI}$ vs b')
	...
	fit_legend = ax.legend(handles=fit_objects, loc='lower left')
	fit_legend.set_title('EAGLE Fits',prop={'size':15})
	plt.rcParams['legend.fontsize'] = 15.
	ax.add_artist(fit_legend)
	plt.tight_layout()

	if virial_radii_bool:
		plt.savefig(ion + '_equ_width_contour_test_virial.pdf', bbox_inches='tight')
	else:
		plt.savefig(ion + '_equ_width_contour_test_kpc.pdf', bbox_inches='tight')
	plt.close()
...
def plot_for_multiple_gals_by_radius(ion, radii_bins, plot_colors, virial_vel_bool, virial_radii_bool, halo_mass_bool, mean_spectra_bool, radii, virial_radii, masses, smasses, flux_for_stacks, vel_for_stacks, virial_vel_for_stacks, min_halo_mass, max_halo_mass):
	...
	plt.title('Flux vs Speed Relative to Central Galaxy for %s [%.1f-%.1f]' % (ion.upper().upper(), min_halo_mass,max_halo_mass) )
	...
	if virial_radii_bool:
		if virial_vel_bool:
			if mean_spectra_bool:
				plt.savefig(ion.upper() +'radius_bins_mean_spectra_virial_radius_virial_vel.pdf')
			else:
				plt.savefig(ion.upper() +'radius_bins_median_spectra_virial_radius_virial_vel.pdf')
		else:
			if mean_spectra_bool:
				plt.savefig(ion.upper() +'radius_bins_mean_spectra_virial_radius_physical_vel.pdf')
			else:
				plt.savefig(ion.upper() +'radius_bins_median_spectra_virial_radius_physical_vel.pdf')
	else:
		if virial_vel_bool:
			if mean_spectra_bool:
				plt.savefig(ion.upper() +'radius_bins_mean_spectra_physical_radius_virial_vel.pdf')
			else:
				plt.savefig(ion.upper() +'radius_bins_median_spectra_physical_radius_virial_vel.pdf')
		else:
			if mean_spectra_bool:
				plt.savefig(ion.upper() +'radius_bins_mean_spectra_physical_radius_physical_vel.pdf')
			else:
				plt.savefig(ion.upper() +'radius_bins_median_spectra_physical_radius_physical_vel.pdf')
...
def plot_for_multiple_gals_by_mass(ion, mass_bins, plot_colors, virial_vel_bool, virial_radii_bool, halo_mass_bool, mean_spectra_bool, masses, smasses, radii, virial_radii, flux_for_stacks, vel_for_stacks, virial_vel_for_stacks, min_halo_mass, max_halo_mass):
	...
	plt.grid()
	plt.ylim(ymin=-0.2, ymax=1)
	plt.title('Flux vs Speed Relative to Central Galaxy for %s [%.1f-%.1f]' % (ion.upper(), min_halo_mass,max_halo_mass) )
	plt.ylabel('normalized flux')
	plt.hold(False)
	if virial_vel_bool:
		plt.xlim(xmin=-3, xmax = 3)
...
def get_line_kinematics(flux, velocity, temperature, ion_densities, nH, optical_depth, gals_num, spec_num, radius, gal_mass, gal_ssfr, col_dense, single_line_temp_bins, single_line_max_plots, single_line_num_plotted, make_realistic_bool, rest_wavelength=None, redshift=None, directory_with_COS_LSF='./'):
	...
	# 		do_plot = False
	# 	else:
	# 		plot_temp_index = bisect.bisect_left(single_line_temp_bins, np.log10(np.max(temps)))
	# 		try:
	# 			single_line_num_plotted[plot_temp_index] += 1
	# 		except:
	# 			print np.log10(temps)
	# 			print single_line_temp_bins
	# 			print ''
	# 		if single_line_num_plotted[plot_temp_index] > single_line_max_plots:
	# 			do_plot = False
	# 	if do_plot:
	# 		for i in range(0,np.size(min_indices)):
	# 			plt.plot(velocity, flux, 'k', label = 'velocity=%d, FWHM=%d, temp=%.2f' % (centroid_vel[i], FWHM[i], np.log10(temps[i])))
...
def neutral_columns_plot(cols, H_cols, radii, virial_radii, R200, smasses, masses, ssfr, ion_num_densities, gas_densities, temps, mean_bool, virial_radii_bool, pop_str):
	# ### if we only want to look at hot or cool gas
	# temp_indices = np.where(np.log10(temps) > 5.0)
	# cols, H_cols, radii, virial_radii, R200, smasses, masses, ssfr, ion_num_densities, gas_densities, temps = cols[temp_indices], H_cols[temp_indices], radii[temp_indices], virial_radii[temp_indices], R200[temp_indices], smasses[temp_indices], masses[temp_indices], ssfr[temp_indices], ion_num_densities[temp_indices], gas_densities[temp_indices], temps[temp_indices]
...
def kinematic_plots(num_minima, centroid_vel, depth, FWHM, radii, temps, line_ion_densities, line_nHs, escape_vels, virial_radii_for_kin, halo_masses_for_kin, stellar_masses_for_kin, ssfr_for_kin, redshifts_for_kin, bins_for_median):
	...
	mask[np.where(FWHM <0)] = False
	mask[np.where(FWHM>1.e4)] = False
	...
	# ### Histograms
	plt.rcParams['axes.labelsize'], plt.rcParams['axes.titlesize'], plt.rcParams['legend.fontsize'], plt.rcParams['xtick.labelsize'], plt.rcParams['ytick.labelsize'] = 25., 30., 25., 20, 20
	...
	make_2d_hist_plot(np.log10(halo_masses_for_kin), temps_in_vir, hist_bins, 'mass_vir_temp_hist_%s.pdf' % ('all'), stellar_masses_for_kin, ssfr_for_kin, log_y=False, \
		plot_labels=[r'$M_{200}$ vs Temperature', r'${\rm log_{10}}(M_{200})$ $M_{\odot}$', r'$log_{10}(T/T_{200})$', r'$log_{10}(N_{components})$'], median=True, median_along='x',
	...
	make_2d_hist_plot(np.log10(halo_masses_for_kin), temps, hist_bins, 'mass_temp_hist_%s.pdf' % ('all'), stellar_masses_for_kin, ssfr_for_kin, log_y=False, \
		plot_labels=[r'$M_{200}$ vs Temperature', r'${\rm log_{10}}(M_{200})$ $M_{\odot}$', r'$log_{10}(T)$ K', r'$log_{10}(N_{components})$'], median=True, median_along='x',
...
def make_2d_hist_plot(xvals, yvals, hist_bins, name, smass, ssfr, plot_labels = ['title', 'x label', 'y label', 'cbar label'], log_cbar = True, log_x = False, log_y=False, median=False, median_along='x', 
	...
	if populations_data[0] != None:
		...
		for i in range(0,np.size(upper_mass)):
			...
			# axHistx.hist(xvals[indices], bins=hist_bins, alpha=0.33, color=colors[i], normed=True, range=(xlims[0],xlims[1]), label=labels[i])
	...
	axMain.hold(False)
	axMain.set_title(plot_labels[0], x=0.7,  y=1.27)
	...
	# axHistx.legend(loc=(0.45,-1.3))
	axHistx.tick_params(labelbottom=False)
	axHistx.tick_params(labelleft=False)
	axHisty.tick_params(labelbottom=False)
...
def handle_single_realization_statistics(spec_output_directory, cos_smass_data, cos_ssfr_data, cos_id_arr, cos_h1_equ_widths, cos_h1_W_flags, cos_h1_equ_widths_radii, cos_h1_cols, cos_h1_cols_flags, cos_h1_cols_radii, equ_widths_bool):
	...
	for j in range(0,np.size(upper_mass)):
		...
		for i in range(0,num_runs):
			...
			### old (2d) added return of the cos/sim values needed
			reject_val, cos_Ws, cos_cols, cos_radii, sim_Ws, sim_cols, sim_radii = get_KS_for_single(curr_directory_arr, curr_file_ids, curr_cos_ids, curr_spec_nums, curr_cos_id_arr, curr_cos_h1_equ_widths, \
			...
			# 	upper_radii = cos_radii[curr_cos_h1_W_flags == 3]
			# 	upper_Ws = cos_Ws[curr_cos_h1_W_flags == 3]
			...
			# 	ax.set_xlim(xlim)
			# 	ax.set_ylim(ylim)
			# 	fig.savefig('%s_ws_%s.pdf' % (groups[j], str(plot_num)), bbox_inches='tight')
			# 	plt.close(fig)
			...
			# 	norm_radii = cos_radii[curr_cos_h1_cols_flags == 1]
			# 	norm_cols = cos_cols[curr_cos_h1_cols_flags == 1]
			# 	upper_radii = cos_radii[curr_cos_h1_cols_flags == 3]
			# 	upper_cols = cos_cols[curr_cos_h1_cols_flags == 3]
			# 	lower_radii = cos_radii[curr_cos_h1_cols_flags == 2]
			# 	lower_cols = cos_cols[curr_cos_h1_cols_flags == 2]
			...
			# 	ax.set_xlabel('Impact Parameter (kpc)')
			# 	ax.set_ylabel(r'${\rm log}_{10}(N_{HI})$ $cm^{-2}$')
			# 	ax.set_xlim(xlim)
			# 	ax.set_ylim(ylim)
			# 	fig.savefig('%s_cols_%s.pdf' % (groups[j], str(plot_num)), bbox_inches='tight')
			# 	plt.close(fig)
...
def get_KS_for_single(curr_directory_arr, curr_file_ids, curr_cos_ids, curr_spec_nums, cos_id_arr, cos_h1_equ_widths, cos_Ws_flags, \
	...
	if equ_widths_bool:
		...
		# ax.set_axisbelow(True)
		# ax.legend()
		# ax.set_title('%s Example: Reject at %s%% certainty' % (plot_group ,str(reject_val)))
		# ax.set_xlabel('Impact Parameter (kpc)')
		# ax.set_ylabel(r'Equivalent Width ($\AA{}$)')
		# ax.set_xlim(xlim)
		# ax.set_ylim(ylim)
		# fig.savefig('%s_ws_%s.pdf' % (plot_group, str(plot_num)), bbox_inches='tight')
		# plt.close(fig)
		...
		# ax.set_xlim(xlim)
		# ax.set_ylim(ylim)
		# fig.savefig('%s_cols_%s.pdf' % (plot_group, str(plot_num)), bbox_inches='tight')
		# plt.close(fig)

	return reject_val, cos_Ws, cos_cols, cos_radii, sim_Ws, sim_cols, sim_radii
